...
public class PlayerController : MonoBehaviour
{
    ...
    //gère tout les mouvement selon le boutton appuyé 
    void Update()
    {
        if (!stopMovement)
        {
            ...
            if (Input.GetButtonDown("Fire1")) //si nous appuyons sur "T" une fois pour tirer (configuré dans Project Settings -> positive button)
            {
                if (!doubleShotActive) //dans le cas où nous avons récupérer le bonus D
                {
                    Instantiate(shot, shotPoint.position, shotPoint.rotation);
                }
                else //sinon tir basique
                {
            ...
            if (Input.GetButton("Fire1")) // si on appuie longuement sur "T" pour tirer afin de faire des tirs continus
            {
