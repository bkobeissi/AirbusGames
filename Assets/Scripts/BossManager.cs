...
public class BossManager : MonoBehaviour
{
    ...
    public int currentHealth = 100; //vie du Boss
    ...
    void Update()
    {
        if (!battleEnding) //dans le cas où le combat est terminé
        {
    ...
    //quand le joueur touche le boss et donc lui fait perdre de la vie (qui est de 100)
    public void HurtBoss()
    {
        currentHealth--;
        UIManager.instance.bossHealthSlider.value = currentHealth; //on gère la barre de vie du boss qui diminue

        if (currentHealth <= 0 && !battleEnding) //dans le cas où le boss n'a plus de vie et donc le joueur gagne sa partie
        {
            battleEnding = true;
    ...
    public IEnumerator EndBattleCo() //fin du level avec le boss
    {
